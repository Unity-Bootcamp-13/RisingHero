...
public class SkillCaster123 : MonoBehaviour
{
    [SerializeField] private CharacterStats123 characterStats; // 시전자의 스탯 (MP, 공격력 etc)
    [SerializeField] private TargetManager123 targetManager;   // 현재 타겟 관리
    ...
    public CharacterStats123 CharacterStats => characterStats;
    public TargetManager123 TargetManager => targetManager;

    private void Awake()
    {
        if (characterStats == null)
            characterStats = GetComponent<CharacterStats123>();
        if (targetManager == null)
            targetManager = GetComponent<TargetManager123>();
    ...
    public bool TryCastSkill(SkillDefinition skillToCast, Vector3? targetPosition = null)
    {
        ...
        // 3. 타겟이 유효한가?
        Targetable123 currentTarget = targetManager.GetCurrentTarget();
    ...
    private void StartSkillExecution(SkillDefinition skillToCast, Targetable123 target, Vector3? castPosition)
    {
